    ...
    func designTagButton(){
        ...
        var result: [String] = []
            ...
            while result.count < 4 {
                let appendValue = newlyCoinedWordDictionary.keys.randomElement()!
                if !result.contains(appendValue) {
                    result.append(appendValue)
                }
            }
        
        for tag in 0...tagButton.count-1 {
                tagButton[tag].layer.borderWidth = 2
                tagButton[tag].layer.cornerRadius = 10
                tagButton[tag].setTitle(" \(result[tag]) ", for: .normal)
            }
    }
